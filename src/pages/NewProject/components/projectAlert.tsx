...
const ProjectAlert = (props : prop) : JSX.Element =>{
  ...
  const handleButtonClick = () => {
    ...
    newText.textContent = (document.getElementById("mailInput") as HTMLInputElement).value;
    newText.animate([
      {
  ...
  return (
    <div className={classes.root}>
      <Typography>
        이 옵션은 프로젝트에서 특정한 레벨의 이벤트가 발생했을 때, 그 상황에 대한 알림을 이메일로 받아볼 수 있게 설정할 수 있습니다.
      </Typography>
      <Grid container>
        <Grid className={classes.radios} item xs={12} sm={3}>
          <FormControl component="fieldset">
            <FormLabel component="legend">동의여부</FormLabel>
            <RadioGroup aria-label="alert" name="alert" value={props.alertSetting} onChange={handleChange}>
              <FormControlLabel value="동의" control={<Radio />} label="동의" onClick={animationsPlay} />
              <FormControlLabel value="거부" control={<Radio />} label="거부" onClick={animationsPause} />
            </RadioGroup>
          </FormControl>
        </Grid>
        <Grid item xs={12} sm={3}>
          ...
          <input id="mailInput" className={classes.inputSet} type="text" disabled={props.alertSetting==='거부'} />
          <Button 
